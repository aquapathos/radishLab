...
cdef neighboursC(int x, int y, np.ndarray img):
...
def getSkelline( img):
    cdef np.ndarray thinned
    cdef int x,y,i,rows,columns
    cdef double P2,P3,P4,P5,P6,P7,P8,P9,s
    cdef int ongo, chg1,chg2
    cdef int *changex, *changey
    
    ongo = 1
    thinned = img.copy() / 255
    rows, columns = thinned.shape[0],thinned.shape[1] 
